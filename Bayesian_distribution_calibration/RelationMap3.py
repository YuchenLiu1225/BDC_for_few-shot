...
class RIM(nn.Module):
    ...
    def forward(self, x, mode = "Train", n_gaus = 1000):
        ...
        if self.truee:
            print("bars:", alpha_bars)
            print("tilde", alpha_tilde)
        ...
        if mode == "Train":
            ...
            H_g = torch.transpose(H_g, 0, 1)
            H_g = H_g + x_node_emb
            return H_g, alpha_bars[0] #n_gaus * g_dim
    ...
    def norm(self, x):
        #return 2 * (x - (torch.max(x) + torch.min(x)) / 2) / (torch.max(x) - torch.min(x))
