...
class DermDataset():
    def __init__(self, dataroot, base_num = 10, cudaid = 'cuda:0'):
        ...
        for(img, label) in self.x:
            img = self.modelout(img.unsqueeze(0).to(self.device,torch.float))#这里将图片放到预训练模型里面提取特征，输入（长*宽*3（RGB)),先unsqueeze成一维，然后进入预训练模型提取特征。
            img = img.cpu().detach().numpy().reshape(-1)#提取的特征转变成numpy格式
            #print(img.shape)
            if label in temp:
